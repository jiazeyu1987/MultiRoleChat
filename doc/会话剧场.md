...
会话剧场是一个复杂的多角色对话执行系统，将流程模板转换为交互式对话。它提供从模板选择到对话执行的完整生命周期，具有高级功能如角色映射、上下文管理和实时分步执行。
